...
##### flags #####
flags.DEFINE_string('target_1_fasta', None,
    'Paths to FASTA file for the first target protein')

flags.DEFINE_string('target_2_fasta', None,
    'Path to FASTA file for the second target protein')

flags.DEFINE_list('target_1_if_residues', None,
    'Comma separated list of interface residues for target 1 (start at zero).')

flags.DEFINE_list('target_2_if_residues', None,
    'Comma separated list of interface residues for target 1 (start at zero).')

flags.DEFINE_list('msas', None,
    'Comma separated list of msa paths (paired and bock diagonalized).')
...
def predict_function(peptide_sequence, feature_dict, output_dir, model_runners,
                     random_seed,
                     l1, l2,
                     target_1_if_residues, target_2_if_residues,
                     predict_only):
    ...
    if predict_only==True:
        return 0, 0, 0, unrelaxed_protein
    else:
        ...
        target_1_res_index = residue_index[:l1]
        target_2_res_index = residue_index[l1:-peptide_length]
        peptide_res_index = residue_index[-peptide_length:]
        #Get coords
        target_1_coords = protein_atom_coords[np.argwhere(protein_resno<=target_1_res_index[-1]+1)[:,0]]
        target_2_coords = protein_atom_coords[np.argwhere((protein_resno>target_1_res_index[-1]+1)&(protein_resno<=target_2_res_index[-1]+1))[:,0]]
        peptide_coords = protein_atom_coords[np.argwhere(protein_resno>target_2_res_index[-1]+1)[:,0]]
        #Get atom types
        target_1_atoms = protein_atoms[np.argwhere(protein_resno<=target_1_res_index[-1]+1)[:,0]]
        target_2_atoms = protein_atoms[np.argwhere((protein_resno>target_1_res_index[-1]+1)&(protein_resno<=target_2_res_index[-1]+1))[:,0]]
        peptide_atoms = protein_atoms[np.argwhere(protein_resno>target_2_res_index[-1]+1)[:,0]]
        #Get resno for each atom
        #Start at 1 - same for receptor_if_residues
        target_1_resno = protein_resno[np.argwhere(protein_resno<=target_1_res_index[-1]+1)[:,0]]
        target_2_resno = protein_resno[np.argwhere((protein_resno>target_1_res_index[-1]+1)&(protein_resno<=target_2_res_index[-1]+1))[:,0]]
        peptide_resno = protein_resno[np.argwhere(protein_resno>target_2_res_index[0])[:,0]]


        #Get atoms belonging to if res for each target
        #Target 1
        target_1_if_pos = []
        for ifr in np.unique(target_1_resno)[target_1_if_residues]:
            target_1_if_pos.extend([*np.argwhere(target_1_resno==ifr)])
        target_1_if_pos = np.array(target_1_if_pos)[:,0]
        #Target 2
        target_2_if_pos = []
        for ifr in np.unique(target_2_resno)[target_2_if_residues]:
            target_2_if_pos.extend([*np.argwhere(target_2_resno==ifr)])
        target_2_if_pos = np.array(target_2_if_pos)[:,0]

        #Calc 2-norm - distance between peptide and interface
        #Target 1
        mat = np.append(peptide_coords,target_1_coords[target_1_if_pos],axis=0)
        a_min_b = mat[:,np.newaxis,:] -mat[np.newaxis,:,:]
        dists = np.sqrt(np.sum(a_min_b.T ** 2, axis=0)).T
        pl = len(peptide_coords)
        #Get interface
        contact_dists = dists[:pl,pl:] #first dimension = peptide, second = receptor
        #Get the closest atom-atom distances across the receptor interface residues.
        closest_dists_peptide_t1 = contact_dists[np.arange(contact_dists.shape[0]),np.argmin(contact_dists,axis=1)]

        #Target 2
        mat = np.append(peptide_coords,target_2_coords[target_2_if_pos],axis=0)
        a_min_b = mat[:,np.newaxis,:] -mat[np.newaxis,:,:]
        dists = np.sqrt(np.sum(a_min_b.T ** 2, axis=0)).T
        #Get interface
        contact_dists = dists[:pl,pl:] #first dimension = peptide, second = receptor
        #Get the closest atom-atom distances across the receptor interface residues.
        closest_dists_peptide_t2 = contact_dists[np.arange(contact_dists.shape[0]),np.argmin(contact_dists,axis=1)]

        #Get the peptide plDDT
        peptide_plDDT = plddt[-peptide_length:]

        return closest_dists_peptide_t1.mean(), closest_dists_peptide_t2.mean(), peptide_plDDT.mean(), unrelaxed_protein
...
def save_design(unrelaxed_protein, output_dir, model_name, l1, l2):
    ...
    with open(unrelaxed_pdb_path, 'w') as f:
        ...
        for line in pdb_contents:
            try:
                ...
                if record['res_no']>prev_resno:
                    ri+=1
                    prev_resno = record['res_no']
                    if ri>l1+1:
                        chain_name='B'
                    if ri>(l1+l2+1):
                        chain_name='C'
...
def optimise_binder(
    target_1_fasta: str,
    target_2_fasta: str,
    target_1_if_residues: str,
    target_2_if_residues: str,
    peptide_length: int,
    output_dir: str,
  ...
  #Merge fastas to a single file
  seq1 = read_fasta(target_1_fasta)
  seq2 = read_fasta(target_2_fasta)
  with open(output_dir+'joint.fasta', 'w') as file:
      file.write('>joint\n')
  ...
  feature_dict = data_pipeline.process(
        input_fasta_path=output_dir+'joint.fasta',
        input_msas=FLAGS.msas,
        template_search=None,
        msa_output_dir=None)

  #Update residue index to include a chain break between target protein 1 and 2
  residue_index = feature_dict['residue_index']
  residue_index[l1:]+=200

  #Load target residues
  if target_1_if_residues:
    target_1_if_residues = np.array(target_1_if_residues, dtype=int)
  else:
    print('No target residues provided for target 1. Designing towards entire sequence for target 1...')
    target_1_if_residues = np.arange(l1)

  if target_2_if_residues:
    target_2_if_residues = np.array(target_2_if_residues, dtype=int)
  else:
    print('No target residues provided for target 2. Designing towards entire sequence for target 2...')
    target_2_if_residues = np.arange(l2)
  ...
  sequence_scores = {'iteration':[], 'if_dist_peptide_t1':[], 'if_dist_peptide_t2':[], 'plddt':[], 'loss':[], 'sequence':[]}
  ...
  if len(sequence_scores['if_dist_peptide_t1'])<1:
      #Get an initial estimate
      if_dist_peptide_t1, if_dist_peptide_t2, plddt, unrelaxed_protein = predict_function(peptide_sequence, feature_dict, output_dir, model_runners,
                                                                   random_seed,
                                                                   l1, l2,
                                                                   target_1_if_residues, target_2_if_residues,
                                                                   predict_only)
      ...
      sequence_scores['if_dist_peptide_t1'].append(if_dist_peptide_t1)
      sequence_scores['if_dist_peptide_t2'].append(if_dist_peptide_t2)
      sequence_scores['plddt'].append(plddt)
      loss = (if_dist_peptide_t1+if_dist_peptide_t2)*1/plddt
      sequence_scores['loss'].append(loss)
      sequence_scores['sequence'].append(peptide_sequence)
      print('init', if_dist_peptide_t1, if_dist_peptide_t2, plddt, loss, peptide_sequence)

  #Save
  if predict_only==True:
      save_design(unrelaxed_protein, output_dir, 'true', l1, l2)
  ...
  #Iterate
  for num_iter in range(len(sequence_scores['if_dist_peptide_t1']), num_iterations):
    #Mutate sequence
    new_sequence = mutate_sequence(peptide_sequence, sequence_scores)
    #Predict and get loss
    if_dist_peptide_t1, if_dist_peptide_t2, plddt, unrelaxed_protein = predict_function(new_sequence, feature_dict, output_dir, model_runners,
                                                                                        random_seed,
                                                                                        l1, l2,
                                                                                        target_1_if_residues, target_2_if_residues,
                                                                                        predict_only)
    ...
    save_design(unrelaxed_protein, output_dir, str(num_iter), l1, l2)
...
def main(argv):
  ...
  optimise_binder(
        target_1_fasta=FLAGS.target_1_fasta,
        target_2_fasta=FLAGS.target_2_fasta,
        target_1_if_residues=FLAGS.target_1_if_residues,
        target_2_if_residues=FLAGS.target_2_if_residues,
